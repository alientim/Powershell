...
$form.Size = New-Object System.Drawing.Size(550, 400)  # GroÃŸes komfortables Fenster
...
$ggListBox.SelectionMode = [System.Windows.Forms.SelectionMode]::One  # Corrected this line
...
$dlListBox.SelectionMode = [System.Windows.Forms.SelectionMode]::MultiExtended  # This is correct for multiple selections
...
$okButton.Add_Click({
    ...
    foreach ($dlGroup in $dl) {
        if (-not (Get-ADGroup -Filter "Name -eq '$dlGroup'")) {
            New-ADGroup -Name $dlGroup -GroupScope DomainLocal -GroupCategory Security -Path "OU=DL_Groups,DC=deineDomÃ¤ne,DC=local"
        ...
        if (-not (Get-ADGroupMember -Identity $dlGroup -Recursive | Where-Object { $_.SamAccountName -eq $gg })) {
            ...
            Write-Output "Globalgruppe $gg wurde zur DomÃ¤nenlokalen Gruppe $dlGroup hinzugefÃ¼gt."
        } else {
            Write-Output "Globalgruppe $gg ist bereits Mitglied der DomÃ¤nenlokalen Gruppe $dlGroup."
        }
    }
